...
"GET DATA"
# Open Excel file
# file = XLSX.readxlsx("matrices/20251211_CentralArctic.xlsx")
# location = "Arctic"
file = XLSX.readxlsx("matrices/20251212_GreenlandSea.xlsx")
location = "GreenlandSea"
...
A = Float64.(coalesce.(XLSX.getdata(sheet, "F24:BI79") .|> x -> x === nothing ? missing : x, 0.0))
A=copy(transpose(A)) #collumn eats row
N = size(A,1)
all_names = XLSX.getdata(sheet, "F7:BI7")
all_ids = vec(Int64.(coalesce.(XLSX.getdata(sheet, "F8:BI8"), 0))) # Takes ID, sets missing IDs to 0, converts into Int64
# all_ids = vec(parse.(Int64, string.(coalesce.(XLSX.getdata(sheet, "F8:AX8"), "0"))))
all_alpha = XLSX.getdata(sheet, "F18:BI18")
all_producer = XLSX.getdata(sheet, "F19:BI19")
# all_producer = Float64.(all_alpha .== 0)


#define list of all taxa with primary producer info
...
communities = [metacomm, noCyano, noBacteria, noDetri]

@save "$(location)_options.jld2" communities
